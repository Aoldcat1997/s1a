...
class Nansat(Domain, Exporter):
    ...
    * and much more...
    ...
    def sea_land_mask(self, 
                  ...
                  **kwargs):
        """
        创建海陆海陆掩膜（支持多数据源、可指定掩膜类型、自动适配分辨率）

        支持的数据源：
        - 'mod44w'：MODIS 44W产品（250m分辨率，需提前下载数据）
        - 'landmask'：使用global_land_mask库的海岸线数据（无需需额外文件）
        - 'custom'：自定义掩膜文件（需通过kwargs传入`custom_path`）

        参数
        -----------
        mask_source : str
            掩膜数据源，可选：'mod44w'（默认）、'landmask'、'custom'
        mod44path : str
            MOD44W数据路径（仅当mask_source='mod44w'时有效），默认从环境变量MOD44WPATH获取
        dst_domain : Domain
            目标投影区域，默认使用当前Nansat对象的区域
        mask_type : str
            掩膜类型：'water'（水体=1，陆地=0）或'land'（陆地=1，水体=0），默认'water'
        resolution : float
            目标分辨率（米），自动适配目标区域（None则使用数据源原生分辨率）
        cache_dir : str
            缓存目录，用于存储临时重投影结果（加速重复调用）
        **kwargs : 
            传递给reproject的参数（如tps、skip_gcps、resample_alg等）

        返回
        --------
        mask_nansat : Nansat对象
            重投影到目标区域的掩膜数据，掩膜值为1/0（对应mask_type指定的类型）
        """
        # 校验输入参数
        valid_sources = ['mod44w', 'landmask', 'custom']
        if mask_source not in valid_sources:
            raise ValueError(f"mask_source必须为{valid_sources}之一，当前为{mask_source}")
        if mask_type not in ['water', 'land']:
            raise ValueError(f"mask_type必须为'water'或'land'，当前为{mask_type}")
        ...
        self.logger.debug(f"掩膜目标区域：经度[{lon_min:.2f},{lon_max:.2f}]，纬度[{lat_min:.2f},{lat_max:.2f}]")

        # 根据数据源加载原始掩膜
        mask_nansat = self._load_mask_source(
        ...
        if cache_dir is not None:
            cache_key = self._get_cache_key(dst_domain, mask_source, resolution)
            cache_path = os.path.join(cache_dir, f"mask_{cache_key}.vrt")
            if os.path.exists(cache_path):
                self.logger.debug(f"从缓存加载掩膜：{cache_path}")
                return Nansat(cache_path, log_level=self.logger.level)

        # 裁剪并重新投影
        mask_nansat.crop_lonlat([lon_min, lon_max], [lat_min, lat_max])
        mask_nansat.reproject(dst_domain, addmask=False,** kwargs)

        # 缓存结果（如果指定了缓存目录）
        if cache_dir is not None:
            os.makedirs(cache_dir, exist_ok=True)
            mask_nansat.export(cache_path, driver='VRT')
            self.logger.debug(f"掩膜缓存至：{cache_path}")

        # 转换掩膜类型（如果需要陆地掩膜，反转水体掩膜）
        if mask_type == 'land':
            water_band = mask_nansat[1]
            land_band = 1 - water_band
            mask_nansat.add_band(land_band, parameters={'name': 'land_mask', 'wkv': 'land_binary_mask'})
            mask_nansat.vrt.remove_band(1)  # 删除原始水体掩膜波段

        return mask_nansat


    def _load_mask_source(self, mask_source, mod44path, custom_path, lon_min, lon_max, lat_min, lat_max, resolution):
        """加载原始掩膜数据源（内部辅助方法）"""
        if mask_source == 'mod44w':
            # 兼容原有MOD44W数据源
            mod44DataExist = True
            ...
            elif not os.path.exists(os.path.join(mod44path, 'MOD44W.vrt')):
                mod44DataExist = False

            if not mod44DataExist:
                raise IOError(
                    "MOD44W数据不存在，请从http://www.glcf.umd.edu/data/watermask/下载，"
                    f"或设置环境变量MOD44WPATH指向数据目录（当前路径：{mod44path}）"
                )
            self.logger.debug(f"加载MOD44W数据：{mod44path}/MOD44W.vrt")
            return Nansat(os.path.join(mod44path, 'MOD44W.vrt'), mapper='MOD44W', log_level=self.logger.level)

        elif mask_source == 'landmask':
            # 使用global_land_mask库生成掩膜（无需额外文件）
            try:
                from global_land_mask import Globe
            except ImportError:
                raise ImportError("使用'landmask'数据源需先安装：pip install global-land-mask")
            ...
            # 根据分辨率计算网格步长（度）
            # 1度≈111000米，分辨率转换为度
            res_deg = resolution / 111000 if resolution else 0.01
            # 扩展网格范围避免边界误差
            lon_grid = np.arange(lon_min - res_deg, lon_max + res_deg, res_deg)
            lat_grid = np.arange(lat_min - res_deg, lat_max + res_deg, res_deg)
            lon_grid, lat_grid = np.meshgrid(lon_grid, lat_grid)

            # 生成陆地掩膜（1=陆地，0=海洋）- global_land_mask的is_land接受(lat, lon)
            land_array = globe.is_land(lat_grid, lon_grid).astype(np.int8)
            water_array = 1 - land_array  # 转为水体掩膜（统一后续处理）

            # 包装为Nansat对象：使用Domain类的srs和ext参数格式
            from nansat.domain import Domain
            # 构造ext参数（GDAL风格的范围和尺寸定义）
            # -lle: 经纬度范围（lonmin, latmin, lonmax, latmax）
            # -ts: 网格尺寸（width, height）
            ext_params = (
                f"-lle {lon_min - res_deg} {lat_min - res_deg} {lon_max + res_deg} {lat_max + res_deg} "
                f"-ts {lon_grid.shape[1]} {lon_grid.shape[0]}"
            )
            # 创建Domain实例（srs为WGS84地理坐标系，ext为范围和尺寸参数）
            domain = Domain(
                srs="+proj=longlat +datum=WGS84",  # PROJ4格式的空间参考
                ext=ext_params                     # 符合GDAL格式的范围和尺寸参数
            )
            mask_nansat = Nansat.from_domain(domain, array=water_array, parameters={
                'name': 'water_mask', 'wkv': 'water_binary_mask'
            })
            self.logger.debug(
                f"生成global_land_mask掩膜：分辨率{resolution or '默认'}米，"
                f"尺寸{water_array.shape}，区域[{lon_min:.2f},{lon_max:.2f}]x[{lat_min:.2f},{lat_max:.2f}]"
            )
            return mask_nansat

        elif mask_source == 'custom':
            # 加载自定义掩膜文件（支持GDAL可识别的格式：GeoTIFF、VRT等）
            if custom_path is None or not os.path.exists(custom_path):
                raise FileNotFoundError(f"自定义掩膜文件不存在：{custom_path}")
            self.logger.debug(f"加载自定义掩膜：{custom_path}")
            return Nansat(custom_path, log_level=self.logger.level)


    def _get_cache_key(self, domain, mask_source, resolution):
        """生成缓存键（基于区域、数据源和分辨率）"""
        from geohash import encode  # 需要安装geohash：pip install geohash
        corners = domain.get_corners()
        geohash = encode((corners[1].mean(), corners[0].mean()), precision=8)
        res_key = f"{resolution:.0f}m" if resolution else "native"
        return f"{geohash}_{mask_source}_{res_key}"
    ...
    def write_geotiffband(self, filename, band_id=None):
        ...
        band_id : int or str
            Band number or name.
        scale_to_byte : bool, default True
            If True, scale data to 0-255 and save as 8-bit.
        apply_log : bool, default False
            If True, apply 10*log10 transform to data.
        nan_to_zero : bool, default False
            If True, set np.nan values to zero before saving.
    ...
    def write_geotiff(self, filename, band_id=1):
        ...
        outDataset = None
        self.vrt.copyproj(filename)
    def write_geotiff_mutilband(self, filename, band_num=1):
        """Writes an 8-bit GeoTiff origion data for a given band.
        ...
        """
        for band_id  in range(band_num):
            bandNo = self.get_band_number(band_id+1)
            band = self.get_GDALRasterBand(band_id+1)
        ...
        for band_id  in range(band_num):
            ...
            data = self.__getitem__(bandNo)
            # scaledData = ((data - bMin) / (bMax - bMin)) * 255
            outDataset.GetRasterBand(band_id+1).WriteArray(data)
            outDataset.GetRasterBand(band_id+1).SetMetadata(band.GetMetadata())
            try:
                outDataset.GetRasterBand(band_id+1).SetColorTable(colorTable)
            except:
                # Happens after reprojection, a possible bug?
                print('Could not set color table')
                print(colorTable)
